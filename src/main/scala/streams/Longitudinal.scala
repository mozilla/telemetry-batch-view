...
case class Longitudinal() extends DerivedStream {
  ...
  private def buildSchema: Schema = {
    ...
    val builder = SchemaBuilder
      .record("Submission").fields()
        ...
        .name("asyncPluginInit").`type`().optional().array().items().booleanType()
        .name("flashVersion").`type`().optional().array().items().stringType()
        .name("previousBuildId").`type`().optional().array().items().stringType()
        .name("previousSessionId").`type`().optional().array().items().stringType()
        .name("previousSubsessionId").`type`().optional().array().items().stringType()
        .name("profileSubsessionCounter").`type`().optional().array().items().intType()
        .name("reason").`type`().optional().array().items().stringType()
        .name("revision").`type`().optional().array().items().stringType()
        .name("sessionId").`type`().optional().array().items().stringType()
        .name("sessionLength").`type`().optional().array().items().longType()
        .name("sessionStartDate").`type`().optional().array().items().stringType()
        .name("subsessionCounter").`type`().optional().array().items().intType()
        .name("subsessionId").`type`().optional().array().items().stringType()
        .name("subsessionLength").`type`().optional().array().items().longType()
        .name("subsessionStartDate").`type`().optional().array().items().stringType()
        .name("timezoneOffset").`type`().optional().array().items().intType()
  ...
  private def buildRecord(history: Iterable[Map[String, Any]], schema: Schema): Option[GenericRecord] = {
    ...
    try {
      ...
      JSON2Avro("payload.info",               List("asyncPluginInit"),          "asyncPluginInit", sorted, root, schema)
      JSON2Avro("payload.info",               List("flashVersion"),             "flashVersion", sorted, root, schema)
      JSON2Avro("payload.info",               List("previousBuildId"),          "previousBuildId", sorted, root, schema)
      JSON2Avro("payload.info",               List("previousSessionId"),        "previousSessionId", sorted, root, schema)
      JSON2Avro("payload.info",               List("previousSubsessionId"),     "previousSubsessionId", sorted, root, schema)
      JSON2Avro("payload.info",               List("profileSubsessionCounter"), "profileSubsessionCounter", sorted, root, schema)
      ...
      JSON2Avro("payload.info",               List("sessionId"),                "sessionId", sorted, root, schema)
      JSON2Avro("payload.info",               List("sessionLength"),            "sessionLength", sorted, root, schema)
      JSON2Avro("payload.info",               List("sessionStartDate"),         "sessionStartDate", sorted, root, schema)
      JSON2Avro("payload.info",               List("subsessionCounter"),        "subsessionCounter", sorted, root, schema)
      JSON2Avro("payload.info",               List("subsessionId"),             "subsessionId", sorted, root, schema)
      JSON2Avro("payload.info",               List("subsessionLength"),         "subsessionLength", sorted, root, schema)
      JSON2Avro("payload.info",               List("subsessionStartDate"),      "subsessionStartDate", sorted, root, schema)
      JSON2Avro("payload.info",               List("timezoneOffset"),           "timezoneOffset", sorted, root, schema)
