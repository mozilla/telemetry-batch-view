...
object AddonRecommender {
  ...
  private class Conf(args: Array[String]) extends ScallopConf(args) {
    ...
    val recommend = new Subcommand("recommend") {
      ...
      val addons = opt[String]("addons", descr = "Command separated list of addons", required = true)
  ...
  private def itemFactorsToMatrix(itemFactors: Array[ItemFactors]): DenseMatrix[Double] = {
    ...
    new DenseMatrix(itemFactors(0).features.size, itemFactors.length, itemMatrixValues)
  }

  private def recommend(inputDir: String, addons: Set[String]) = {
    val mapping: List[(Int, String)] = for {
      JObject(addons) <- parse(Source.fromFile(s"$inputDir/addon_mapping.json").mkString)
      JField(x, JString(y)) <- addons
    } yield {
      (x.toInt, y)
    }
    ...
    val clientAddons = sqlContext.sql("select * from longitudinal")
      ...
      .flatMap{ case Addons(Some(clientId), Some(addons)) =>
        for {
          ...
          tipo <- meta.`type`
          if !blocklisted && (tipo != "extension" || signedState == 2) && !userDisabled && !appDisabled
        } yield {
          (clientId, addonName, hash(clientId), hash(addonName))
        }
      }

    val fakeUsers = sc.parallelize(List(Rating(0, hash("Firebug"), 1.0f))).toDF
