...
object Histograms {
  ...
  private val histogramLocations = List(
    ...
    HistogramLocation(List("payload", "processes", "content", "histograms"), "_CONTENT")
  )

  private val keyedHistogramLocations = List(
    HistogramLocation(List("payload.keyedHistograms"), ""),
    HistogramLocation(List("payload", "processes", "content", "keyedHistograms"), "_CONTENT")
  )
  ...
  private def parseHistogramLocation[HistogramFormat : Manifest](
    ...
    for {
      ...
      root = parse(json.asInstanceOf[String]).extract[JValue]
      // Traverse the rest of the histogram path
      leaf = location.path.tail.foldLeft(root)((acc, x) => acc \ x)
    } yield (
      leaf.extract[Map[String, HistogramFormat]]
