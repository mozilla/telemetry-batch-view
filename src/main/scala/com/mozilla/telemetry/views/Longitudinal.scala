...
object LongitudinalView {
  ...
  private def run(opts: Opts, messages: RDD[Message]): Unit = {
    ...
    val clientMessages = messages
      .flatMap {
        (message) =>
          val fields = message.fieldsAsMap + ("payload" -> message.payload.getOrElse(""))
          for {
  ...
  private def buildSchema(scalarDefs:ScalarsDefinitionApi = Scalars): Schema = {
    ...
    scalarDefs.definitions.foreach{ case (k, value) =>
  ...
  private def getParquetFriendlyScalarName(scalarName: String): String = {
    ...
    // all scalars.
    ScalarColumnNamePrefix + scalarName.replace('.', '_')
  }
  private def scalars2Avro(payloads: List[Map[String, Any]], root: GenericRecordBuilder,
                           scalarDefs:ScalarsDefinitionApi) {
    implicit val formats = DefaultFormats

    // Get a list of the scalars in the ping.
    val scalarList = payloads.map{ case (x) =>
      val json = x.getOrElse("payload.processes.parent.scalars", return).asInstanceOf[String]
      // We can have both string and numerical scalars. Use AnyVal as a placeholder.
      parse(json).extract[Map[String, AnyVal]]
    }
    ...
    val validKeys = for {
      key <- uniqueKeys
      definition <- scalarDefs.definitions.get(key)
    } yield (key, definition)
  ...
  private def keyedScalars2Avro(payloads: List[Map[String, Any]], root: GenericRecordBuilder,
    ...
    val scalarsList = payloads.map{ case (x) =>
      val json = x.getOrElse("payload.processes.parent.keyedScalars", return).asInstanceOf[String]
      parse(json).extract[Map[String, Map[String, AnyVal]]]
    }
  ...
  private def buildRecord(history: Iterable[Map[String, Any]], schema: Schema,
                          scalarDefs:ScalarsDefinitionApi = Scalars): Option[GenericRecord] = {
    // De-dupe records
    ...
    try {
      ...
      scalars2Avro(sorted, root, scalarDefs)
      keyedScalars2Avro(sorted, root, scalarDefs)
