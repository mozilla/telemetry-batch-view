...
object LongitudinalView {
  ...
  private def run(opts: Opts, messages: RDD[Message]): Unit = {
    ...
    val lockfile = new java.io.File(lockfile_name)
    lockfile.createNewFile()
    S3Store.uploadFile(lockfile, outputBucket, prefix, lockfile_name)
    ...
    S3Store.deleteKey(outputBucket,  s"${prefix}/${lockfile_name}")
  }
