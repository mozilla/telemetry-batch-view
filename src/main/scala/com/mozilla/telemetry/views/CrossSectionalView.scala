...
class Longitudinal (
  ...
  private def filterByDOW(session_length: Seq[Long], dow: Int) = {
    (session_length zip this.parsedSubmissionDate).filter(_._2.getDay == dow).map(_._1)
  }

  def activeHoursByDOW(dow: Int) = {
    this.filterByDOW(this.session_length.getOrElse(Seq()), dow).sum/3600.0
  }
}
