...
class Longitudinal (
  ...
  def distinctConfigs[A](getter: (Longitudinal) => Seq[A]) = {
    getter(this).distinct.length
  }
  ...
  private def filterByDOW[A](getter: Longitudinal => Seq[A], dow: Int): Seq[A] = {
    (getter(this) zip this.parsedSubmissionDate).filter(_._2.getDay == dow).map(_._1)
  }

  def activeHoursByDOW(dow: Int) = {
    this.filterByDOW(_.session_length.getOrElse(Seq()), dow).sum/3600.0
  }
}
...
class CrossSectional (
  ...
  def this(base: Longitudinal) = {
    this(
      ...
      geo_Cfgs = base.distinctConfigs(_.geo_country.getOrElse(Seq())),
