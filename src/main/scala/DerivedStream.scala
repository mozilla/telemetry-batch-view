...
import telemetry.streams.{E10sExperiment, ExecutiveStream, Churn, Longitudinal, Crash}
  ...
  def main(args: Array[String]) {
    ...
    val res = for {
      ...
      (from, ds) <- stream match {
        ...
        case "Crash" =>
          val crashAggregates = Crash()
          Some(options.getOrElse('fromDate, to), crashAggregates)
